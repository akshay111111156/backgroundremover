...
/* ---------- Upload & Remove Background (API Placeholder) ---------- */
uploadInput.addEventListener('change', async (event) => {
  ...
  // 🔹 Step 1: Scroll to preview after upload
  setTimeout(() => {
    previewResultSection.style.display = 'flex';
    previewResultSection.scrollIntoView({ behavior: 'smooth' });
  }, 1000);

  // 🔹 Step 2: Simulate showing result after 2s (like API processing)
  setTimeout(async () => {
...
async function removeBackground(file) {
  ...
  try {
    const response = await fetch('https://api.remove.bg/v1.0/removebg', {
      method: 'POST',
      headers: {
        'X-Api-Key': 'vtYRGM9Esgpj1doq4PUD7avX' 
      },
      body: formData
    });
    ...
    if (!response.ok) {
      ...
      alert("Background removal failed: " + errorText);
      return;
    }
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      console.error("Remove.bg API returned error:", errorJson);
      alert("Background removal failed: " + (errorJson.errors?.[0]?.title || "Unknown error"));
      return;
    }
  ...
  // 🕑 Scroll first to the processed image (smooth transition)
  processedImageSection.scrollIntoView({ behavior: 'smooth', block: 'center' });

  // ⏳ After 1 second, scroll down to background options section
  setTimeout(() => {
    backgroundControls.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, 3000);
};
    ...
    alert("Background removal request failed. Check console for details.");
  }
}
...
sampleImages.forEach(img => {
  img.addEventListener('click', () => {
    ...
    // Step 1: Scroll to preview
    previewResultSection.scrollIntoView({ behavior: 'smooth', block: 'center' });
    ...
    setTimeout(() => {
      ...
      // Step 3: Scroll smoothly to processed image
      processedImageSection.scrollIntoView({ behavior: 'smooth', block: 'center' });

      // Step 4: Then scroll to background controls after a moment
      setTimeout(() => {
        backgroundControls.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 3000);
    }, 3000);
